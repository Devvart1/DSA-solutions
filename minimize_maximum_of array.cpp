...
bool check(vector<int> &nums, int mid, int n) {
  if (nums[0] > mid)
    return false;
// debug(mid)
  vector<long long int> longNums = nums;
  for (int i = 1; i < n; ++i) {

//   debug(nums[i]) debug(nums[i - 1])
    if (longNums[i - 1] < mid) {
      long long int x = abs(longNums[i - 1] - mid);
      longNums[i] -= x;
      longNums[i - 1] += x;
    }
    // debug("after")
    ...
    if (longNums[i] > mid and longNums[i - 1] >= mid)
      return false;
    // if (nums[i] == 0 and nums[i - 1] > mid)
    //   return false;
